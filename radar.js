...
var RadarChart = {
  draw: function(id, data, options) {
    ...
    var config = {
      ...
      paddingX: w,
      paddingY: h,
      colors: d3.scale.linear().domain([5,16]).range(["lightblue","steelblue"]),
      showLevels: true,
    ...
    function buildVisComponents() {
      ...
      vis.legend = vis.svg.append("svg:g").classed("legend", true)
        ...
        .attr("transform", "translate(" +220 + ", " + 0.1*config.h + ")");

      //Crear texto que indica el anio seleccionado
      vis.yearLabel = vis.svg.append("svg:text").classed("yearLabel", true)
        .attr("transform", "translate(" +180+ ", " + 1.15*config.h + ")")
          .text("")
